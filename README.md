...
Put `Customer.csv`, `Transactions.csv`, and `prod_cat_info.csv` inside the `Case Study 1 - Retail Case Study/` folder.
...
- **e-Shop dominates** all store types in both revenue and transaction volume.
- **Electronics** is the top revenue-generating category, especially among male customers.
- A significant number of transactions have **negative amounts**, indicating returns that need to be filtered for clean analysis.
- Customers aged **25–35** are high spenders, particularly in Electronics and Books.
